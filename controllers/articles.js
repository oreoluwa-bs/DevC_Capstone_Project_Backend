...
const commentArticle = (req, res) => {
  ...
  pool.query('SELECT * FROM users WHERE id = $1', [commentPost.authorId])
    .then((resulted) => {
      pool.query('INSERT INTO "commentsArticle"(id, comment, "articleId", "authorId", "createdOn", "authorName") VALUES($1, $2, $3, $4, $5, $6);', [...values, `${resulted.rows[0].firstname} ${resulted.rows[0].lastname}`])
        .then(() => {
          res.status(200).json({
            status: 'success',
          });
